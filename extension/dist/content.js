"use strict";(()=>{function z(r){let e=r.match(/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/issues\/?(?:\?.*)?(?:#.*)?$/);return e?{owner:e[1],repo:e[2]}:null}function $(r){let e=r.match(/github\.com\/([^/]+)\/([^/]+)\/issues\/(\d+)/);if(e)return{owner:e[1],repo:e[2],number:parseInt(e[3],10),type:"issue"};let t=r.match(/github\.com\/([^/]+)\/([^/]+)\/pull\/(\d+)/);return t?{owner:t[1],repo:t[2],number:parseInt(t[3],10),type:"pr"}:null}var A={gray:"#6b7280",yellow:"#eab308",green:"#22c55e",red:"#ef4444",blue:"#3b82f6",purple:"#a855f7",pink:"#ec4899",orange:"#f97316"},D={todo:"To-do",in_progress:"In Progress",complete:"Complete"},N=["todo","in_progress","complete"];var X=["bug","daily"],K="help wanted",J=2e3;function c(r){return chrome.runtime.sendMessage(r)}function G(){return document.documentElement.getAttribute("data-color-mode")==="dark"||document.documentElement.getAttribute("data-dark-theme")==="dark"||document.documentElement.classList.contains("dark")}function P(r){return A[r]??A.gray}var C=class{container;shadow;root;task=null;linkedTasks=[];statuses=[];dropdownOpen=!1;loading=!0;error=null;owner;repo;number;mode;linkedIssueNumbers;labels=[];proposal=null;proposalDraftBody="";proposalBusy=!1;proposalPollHandle=null;proposalNotice=null;destroyed=!1;autoPostEnabled=!0;constructor(e,t,s,o="issue",a=[]){this.owner=e,this.repo=t,this.number=s,this.mode=o,this.linkedIssueNumbers=a,this.container=document.createElement("div"),this.container.id="tasker-status-widget",o==="pr"&&(this.container.style.display="inline-flex",this.container.style.alignItems="center",this.container.style.flexShrink="0",this.container.style.alignSelf="start",this.container.style.position="relative"),this.shadow=this.container.attachShadow({mode:"closed"}),this.root=document.createElement("div"),this.shadow.appendChild(this.root);let n=document.createElement("style");n.textContent=this.mode==="pr"?this.getHeaderStyles():this.getSidebarStyles(),this.shadow.appendChild(n),document.addEventListener("click",l=>{!this.container.contains(l.target)&&this.dropdownOpen&&(this.dropdownOpen=!1,this.render())})}get element(){return this.container}async init(){this.loading=!0,this.error=null,this.render();try{let e=await c({type:"GET_SESSION"});if(!e.ok||!e.data){this.loading=!1,this.error="Not signed in to Tasker",this.render();return}this.mode==="pr"?await this.initPr():await this.initIssue()}catch(e){this.error=e.message??"Connection error"}this.loading=!1,this.render()}async initIssue(){let[e,t,s,o,a]=await Promise.all([c({type:"QUERY_TASK",owner:this.owner,repo:this.repo,number:this.number}),c({type:"QUERY_STATUSES"}),c({type:"QUERY_ISSUE_LABELS",owner:this.owner,repo:this.repo,number:this.number}),c({type:"QUERY_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number}),c({type:"GET_AUTOPOST"})]);e.ok?this.task=e.data??null:this.error=e.error??"Failed to load task",t.ok&&t.data&&(this.statuses=t.data),s.ok&&s.data&&(this.labels=s.data),o.ok&&(this.proposal=o.data??null,this.proposalDraftBody=this.proposal?.body??""),a.ok&&a.data&&(this.autoPostEnabled=a.data.enabled);let n=["queued","drafting","armed","posting"];this.proposal&&n.includes(this.proposal.state)&&this.startProposalPoll(),this.proposal?.state==="posted"&&this.proposal.github_comment_id&&this.verifyPostedComment()}async verifyPostedComment(){if(!this.proposal||this.proposal.state!=="posted")return;let e=this.proposal.id,t=await c({type:"VERIFY_POSTED_COMMENT",proposalId:e});if(!this.destroyed&&!(!t.ok||!t.data)&&t.data.state!==this.proposal?.state){let s=t.data.state==="draft";this.proposal=t.data,this.proposalDraftBody=t.data.body??"",s&&(this.proposalNotice="Previous comment was deleted on GitHub \u2014 reverted to draft.",setTimeout(()=>{this.destroyed||(this.proposalNotice=null,this.render())},8e3)),this.render()}}async initPr(){if(this.linkedIssueNumbers.length===0){this.error="No linked issues found";return}let[e,t]=await Promise.all([c({type:"QUERY_TASKS_BATCH",owner:this.owner,repo:this.repo,issueNumbers:this.linkedIssueNumbers}),c({type:"QUERY_STATUSES"})]);e.ok?this.linkedTasks=e.data??[]:this.error=e.error??"Failed to load tasks",t.ok&&t.data&&(this.statuses=t.data)}render(){this.mode==="pr"?this.renderPr():this.renderSidebar()}renderPr(){let e=G();if(this.root.innerHTML="",this.root.className=`tasker-header ${e?"dark":"light"}`,this.loading){this.root.innerHTML='<button class="tasker-btn" disabled><div class="spinner"></div> Tasker</button>';return}if(this.error){this.root.innerHTML="";return}this.linkedTasks.length!==0&&this.renderPrStatusBadge()}renderPrStatusBadge(){let t=new Set(this.linkedTasks.map(p=>p.status)).size>1,s,o,a;if(t)o=A.purple,a="Mixed";else{let p=this.linkedTasks[0].status;s=this.statuses.find(u=>u.key===p),o=P(s?s.color:"gray"),a=s?.label??p}let n=document.createElement("div");n.className="tasker-wrapper";let l=document.createElement("button");l.className="tasker-btn has-status",l.innerHTML=`
    ...
    `,l.addEventListener("click",p=>{p.stopPropagation(),this.dropdownOpen=!this.dropdownOpen,this.render()}),n.appendChild(l),this.root.appendChild(n),this.dropdownOpen&&this.root.appendChild(this.renderPrDropdown())}renderPrDropdown(){let e=document.createElement("div");e.className="dropdown";let t=document.createElement("div");t.className="linked-notice",t.innerHTML=`Updating <strong>${this.linkedTasks.length}</strong> tracked issue${this.linkedTasks.length>1?"s":""}: ${this.linkedTasks.map(o=>`#${o.issue_number}`).join(", ")}`,e.appendChild(t);let s=this.groupStatuses();for(let o of N){let a=s[o];if(!a||a.length===0)continue;let n=document.createElement("div");n.className="group-label",n.textContent=D[o],e.appendChild(n);for(let l of a){let p=this.linkedTasks.every(f=>f.status===l.key),u=document.createElement("button");u.className=`status-row ${p?"active":""}`,u.innerHTML=`
          <span class="dot" style="background:${P(l.color)}"></span>
          <span class="label">${this.escapeHtml(l.label)}</span>
        `,u.addEventListener("click",async f=>{f.stopPropagation(),await this.updateLinkedStatuses(l.key,l.group_name)}),e.appendChild(u)}}return e}async updateLinkedStatuses(e,t){let s=this.linkedTasks.map(n=>({status:n.status,group:n.status_group}));for(let n of this.linkedTasks)n.status=e,n.status_group=t;this.dropdownOpen=!1,this.render();let o=this.linkedTasks.map(n=>n.issue_number).filter(n=>n!==null),a=await c({type:"UPDATE_LINKED_STATUSES",owner:this.owner,repo:this.repo,issueNumbers:o,status:e,statusGroup:t});a.ok||(this.linkedTasks.forEach((n,l)=>{n.status=s[l].status,n.status_group=s[l].group}),this.error=a.error??"Update failed",this.render(),setTimeout(()=>{this.error=null,this.render()},3e3))}renderSidebar(){let e=G();if(this.root.innerHTML="",this.root.className=`tasker-root ${e?"dark":"light"}`,this.loading){this.root.innerHTML='<div class="section"><div class="header">Tasker</div><div class="spinner-wrap"><div class="spinner"></div></div></div>';return}if(this.error){this.root.innerHTML=`
        <div class="section">
    ...
    `,n.addEventListener("click",l=>{l.stopPropagation(),this.dropdownOpen=!this.dropdownOpen,this.render()}),a.appendChild(n),this.dropdownOpen&&a.appendChild(this.renderDropdown()),this.root.appendChild(a),this.renderProposalPanel()}renderDropdown(){let e=document.createElement("div");e.className="dropdown";let t=this.groupStatuses();for(let s of N){let o=t[s];if(!o||o.length===0)continue;let a=document.createElement("div");a.className="group-label",a.textContent=D[s],e.appendChild(a);for(let n of o){let l=document.createElement("button");l.className=`status-row ${this.task?.status===n.key?"active":""}`,l.innerHTML=`
      ...
      `,t.appendChild(d);let h=document.createElement("div");h.className="proposal-actions";let E=document.createElement("button");E.className="proposal-btn",E.textContent=this.proposalBusy?"Cancelling\u2026":"Cancel",E.disabled=this.proposalBusy,E.title="Stop auto-drafting and keep this as an editable draft.",E.addEventListener("click",()=>void this.cancelAutoDraft()),h.appendChild(E),t.appendChild(h),e.appendChild(t),this.root.appendChild(e);return}if(a){let i=this.proposal?.posted_at?new Date(this.proposal.posted_at).toLocaleString():"";t.innerHTML=`
...
Describe your fix...`,m.value=this.proposalDraftBody,m.disabled=o||this.proposalBusy,t.appendChild(m);let g=document.createElement("div");g.className="proposal-actions";let S=()=>this.proposalDraftBody!==(this.proposal?.body??"")&&this.proposalDraftBody.trim().length>0,b=document.createElement("button");b.className="proposal-btn secondary",b.textContent=this.proposalBusy?"Saving\u2026":this.proposal?"Save changes":"Save draft",b.disabled=this.proposalBusy||o||!S(),b.addEventListener("click",()=>void this.saveProposal()),g.appendChild(b);let k=null;if(o){let i=document.createElement("button");i.className="proposal-btn",i.textContent=s==="posting"?"Posting\u2026":"Disarm",i.disabled=this.proposalBusy||s==="posting",i.addEventListener("click",()=>void this.setProposalState("draft")),g.appendChild(i)}else{let i=document.createElement("button");i.className="proposal-btn primary",i.textContent=this.proposalBusy?"Arming\u2026":"Arm auto-post";let d=S();i.disabled=this.proposalBusy||!this.proposalDraftBody.trim()||d,i.title=d?"Save changes before arming":"",i.addEventListener("click",()=>void this.setProposalState("armed")),g.appendChild(i),k=i}t.appendChild(g);let w=null;if(s!=="posting"){let i=document.createElement("div");i.className="proposal-actions";let d=document.createElement("button");d.className="proposal-btn danger",d.textContent=this.proposalBusy?"Posting\u2026":"Post now";let h=this.proposalDraftBody.trim().length>0;d.disabled=this.proposalBusy||!h,d.title=h?"Post the current text as a comment immediately, without waiting for Help Wanted.":"Type your proposal first.",d.addEventListener("click",()=>void this.postProposalNow()),i.appendChild(d),t.appendChild(i),w=d}if(this.proposal){let i=document.createElement("div");i.className="proposal-actions";let d=document.createElement("button");d.className="proposal-btn",d.textContent=this.proposalBusy?"Clearing\u2026":"Clear draft",d.disabled=this.proposalBusy,d.title="Delete this saved proposal from Tasker and start over.",d.addEventListener("click",()=>void this.clearProposal()),i.appendChild(d),t.appendChild(i)}m.addEventListener("input",()=>{this.proposalDraftBody=m.value;let i=S(),d=this.proposalDraftBody.trim().length>0;b.disabled=this.proposalBusy||o||!i,k&&(k.disabled=this.proposalBusy||!d||i,k.title=i?"Save changes before arming":""),w&&(w.disabled=this.proposalBusy||!d,w.title=d?"Post the current text as a comment immediately, without waiting for Help Wanted.":"Type your proposal first.")});let T=document.createElement("div");if(T.className="proposal-status-line",o)T.textContent=s==="posting"?"Posting now\u2026":'Armed \u2014 waiting for "Help Wanted" label';else if(this.proposal){let i=this.proposal.updated_at?new Date(this.proposal.updated_at).toLocaleString():"";T.textContent=`Draft saved \xB7 ${i}`}else T.textContent='Auto-posts on "Help Wanted" via the poll worker.';if(t.appendChild(T),n&&this.proposal?.last_error){let i=document.createElement("div");i.className="proposal-error",i.textContent=`Last error: ${this.proposal.last_error}`,t.appendChild(i)}e.appendChild(t),this.root.appendChild(e)}async saveProposal(){if(this.proposalBusy)return;this.proposalBusy=!0,this.render();let e=await c({type:"SAVE_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number,body:this.proposalDraftBody});this.proposalBusy=!1,e.ok&&e.data?(this.proposal=e.data,this.proposalDraftBody=e.data.body):(this.error=e.error??"Save failed",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async setProposalState(e){if(this.proposalBusy)return;this.proposalBusy=!0,this.render();let t=await c({type:e==="armed"?"ARM_PROPOSAL":"DISARM_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number});this.proposalBusy=!1,t.ok&&t.data?(this.proposal=t.data,t.data.state==="armed"||t.data.state==="posting"?this.startProposalPoll():this.stopProposalPoll()):(this.error=t.error??"Update failed",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async enqueueAutoDraft(){if(this.proposalBusy)return;this.proposalBusy=!0,this.render();let e=await c({type:"ENQUEUE_AUTO_DRAFT",owner:this.owner,repo:this.repo,number:this.number});this.proposalBusy=!1,e.ok&&e.data?(this.proposal=e.data,this.startProposalPoll()):(this.error=e.error??"Could not start Auto-pilot",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async cancelAutoDraft(){if(this.proposalBusy)return;this.proposalBusy=!0,this.render();let e=await c({type:"CANCEL_AUTO_DRAFT",owner:this.owner,repo:this.repo,number:this.number});this.proposalBusy=!1,e.ok&&e.data?(this.proposal=e.data,this.proposalDraftBody=e.data.body??"",this.stopProposalPoll()):(this.error=e.error??"Could not cancel",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async clearProposal(){if(this.proposalBusy||!confirm("Clear this proposal? This deletes the saved draft from Tasker."))return;this.proposalBusy=!0,this.render();let e=await c({type:"CLEAR_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number});this.proposalBusy=!1,e.ok?(this.proposal=null,this.proposalDraftBody="",this.proposalNotice=null,this.stopProposalPoll()):(this.error=e.error??"Could not clear",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async postProposalNow(){if(!(this.proposalBusy||!this.proposalDraftBody.trim())&&confirm("Post this proposal as a comment now?")){this.proposalBusy=!0,this.render();try{let t=await c({type:"SAVE_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number,body:this.proposalDraftBody});if(!t.ok||!t.data){this.error=t.error??"Save failed",setTimeout(()=>{this.error=null,this.render()},3e3);return}this.proposal=t.data,this.proposal={...this.proposal,state:"posting"},this.startProposalPoll(),this.render();let s=await c({type:"POST_PROPOSAL_NOW",proposalId:t.data.id,force:!0});s.ok&&s.data?(this.proposal=s.data,(s.data.state==="posted"||s.data.state==="failed")&&this.stopProposalPoll()):(this.error=s.error??"Post failed",setTimeout(()=>{this.error=null,this.render()},5e3),this.refreshProposal())}catch(t){console.error("[tasker] postProposalNow threw",t),this.error=t instanceof Error?t.message:"Post failed (channel closed)",setTimeout(()=>{this.error=null,this.render()},5e3)}finally{this.proposalBusy=!1,this.render()}}}startProposalPoll(){this.stopProposalPoll(),this.proposalPollHandle=setInterval(()=>{this.refreshProposal()},J)}stopProposalPoll(){this.proposalPollHandle!==null&&(clearInterval(this.proposalPollHandle),this.proposalPollHandle=null)}async refreshProposal(){if(this.destroyed)return;let e=await c({type:"QUERY_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number});if(this.destroyed||!e.ok||!e.data)return;let t=e.data;(!this.proposal||t.state!==this.proposal.state||t.posted_at!==this.proposal.posted_at)&&(this.proposal=t,(t.state==="posted"||t.state==="failed"||t.state==="draft")&&this.stopProposalPoll(),this.render())}groupStatuses(){let e={todo:[],in_progress:[],complete:[]};for(let t of this.statuses)e[t.group_name]?.push(t);for(let t of N)e[t].sort((s,o)=>s.position-o.position);return e}escapeHtml(e){let t=document.createElement("span");return t.textContent=e,t.innerHTML}destroy(){this.destroyed=!0,this.stopProposalPoll(),this.container.remove()}getHeaderStyles(){return`
    ...
    `}};var W="extensionSettings";var y={autoRefreshEnabled:!1,autoRefreshSeconds:20,notifyHelpWanted:!1,notifyChannels:["browser"],telegramChatId:"",pollSeconds:45,watchedLabelGroups:[["Help Wanted"],["Daily"],["Bug"]],excludedLabels:["DeployBlocker","DeployBlockerCash"],bugDailyPopupEnabled:!0,bugDailyPopupSound:!0},Z=["browser","telegram"];function ee(r){if(!Array.isArray(r))return[...y.notifyChannels];let e=r.filter(t=>typeof t=="string"&&Z.includes(t));return e.length>0?Array.from(new Set(e)):[...y.notifyChannels]}function M(r,e){if(!Array.isArray(r))return[...e];let t=r.filter(s=>typeof s=="string").map(s=>s.trim()).filter(s=>s.length>0&&s.length<=64);return Array.from(new Map(t.map(s=>[s.toLowerCase(),s])).values())}function te(r){if(!Array.isArray(r))return null;let e=[];for(let t of r){if(!Array.isArray(t))continue;let s=M(t,[]);s.length>0&&e.push(s)}return e}async function H(){let e=(await chrome.storage.local.get(W))[W],t,s=te(e?.watchedLabelGroups);return s!==null?t=s:e?.watchedLabels!==void 0?t=M(e.watchedLabels,[]).map(o=>[o]):t=y.watchedLabelGroups.map(o=>[...o]),{autoRefreshEnabled:e?.autoRefreshEnabled??y.autoRefreshEnabled,autoRefreshSeconds:Math.max(5,e?.autoRefreshSeconds??y.autoRefreshSeconds),notifyHelpWanted:e?.notifyHelpWanted??y.notifyHelpWanted,notifyChannels:ee(e?.notifyChannels),telegramChatId:e?.telegramChatId??y.telegramChatId,pollSeconds:Math.max(30,e?.pollSeconds??y.pollSeconds),watchedLabelGroups:t,excludedLabels:e?.excludedLabels===void 0?[...y.excludedLabels]:M(e.excludedLabels,[]),bugDailyPopupEnabled:e?.bugDailyPopupEnabled??y.bugDailyPopupEnabled,bugDailyPopupSound:e?.bugDailyPopupSound??y.bugDailyPopupSound}}function q(r,e,t="seenHelpWanted"){return`${t}:${r.toLowerCase()}/${e.toLowerCase()}`}async function O(r,e,t){let s=q(r,e,t),a=(await chrome.storage.local.get(s))[s]??[];return new Set(a)}async function I(r,e,t,s){let o=Array.from(t),a=o.length>500?o.slice(o.length-500):o;await chrome.storage.local.set({[q(r,e,s)]:a})}var Y="tasker-issue-alert";var v=null;function se(){try{let r=window.AudioContext||window.webkitAudioContext;if(!r)return;v=v??new r,v.state==="suspended"&&v.resume();let e=v.currentTime;for(let[t,s]of[880,1320].entries()){let o=v.createOscillator(),a=v.createGain();o.type="sine",o.frequency.value=s;let n=e+t*.12;a.gain.setValueAtTime(1e-4,n),a.gain.exponentialRampToValueAtTime(.07,n+.02),a.gain.exponentialRampToValueAtTime(1e-4,n+.18),o.connect(a),a.connect(v.destination),o.start(n),o.stop(n+.2)}}catch{}}var oe=`
  .stack {
    position: fixed;
    bottom: 24px;
    left: 50%;
...
`;function re(){let r=document.getElementById(Y);if(r&&r.shadowRoot){let o=r.shadowRoot.querySelector(".stack");return{host:r,stack:o}}r=document.createElement("div"),r.id=Y,Object.assign(r.style,{position:"fixed",top:"0",left:"0",width:"0",height:"0",zIndex:"2147483647"}),document.body.appendChild(r);let e=r.attachShadow({mode:"open"}),t=document.createElement("style");t.textContent=oe;let s=document.createElement("div");return s.className="stack",e.append(t,s),{host:r,stack:s}}function ae(r){let e=new Map;for(let t of r)for(let s of t.split("+")){let o=s.trim();o&&e.set(o.toLowerCase(),o)}return e.size>0?Array.from(e.values()):["New"]}function ne(r){let e=r.toLowerCase();return e.includes("bug")?"chip bug":e.includes("daily")?"chip daily":e.includes("help")?"chip help":e.includes("external")?"chip external":"chip generic"}function ie(r,e){let t=document.createElement("div");t.className="card";let s=document.createElement("div");s.className="flash";let o=document.createElement("div");o.className="hd";let a=document.createElement("span");a.className="bolt",a.textContent="\u26A1";let n=document.createElement("span");n.className="badge",n.textContent="NEW BOUNTY";let l=document.createElement("button");l.className="x",l.textContent="\xD7",l.title="Dismiss",o.append(a,n,l);let p=document.createElement("div");p.className="title",p.textContent=`#${r.number} \xB7 ${r.title}`;let u=document.createElement("div");u.className="chips";for(let k of ae(r.labels)){let w=document.createElement("span");w.className=ne(k),w.textContent=k,u.append(w)}let f=document.createElement("div");f.className="hint",f.textContent="Open it and arm your proposal before someone grabs it.";let m=document.createElement("div");m.className="bar",m.style.animationDuration="15000ms",t.append(s,o,p,u,f,m);let g=null,S=!1,b=()=>{S||(S=!0,g&&clearTimeout(g),t.classList.add("out"),setTimeout(()=>{t.remove(),e()},280))};return t.addEventListener("click",()=>{window.open(r.url,"_blank","noopener"),b()}),l.addEventListener("click",k=>{k.stopPropagation(),b()}),t.addEventListener("mouseenter",()=>{g&&clearTimeout(g),m.style.animationPlayState="paused"}),t.addEventListener("mouseleave",()=>{S||(m.style.animationPlayState="running",g=setTimeout(b,15e3))}),g=setTimeout(b,15e3),t}function L(r,e={}){if(r.length===0)return;let{host:t,stack:s}=re(),o=()=>{s.childElementCount===0&&t.remove()};for(let a of r.slice(0,5))s.appendChild(ie(a,o));e.sound!==!1&&se()}var F="seenBugDailyPopup";function le(r){return r.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function j(r){let e=le(r).replace(/\s+/g,"[\\s-]?");return new RegExp(`(?:^|[^a-z0-9])${e}(?:$|[^a-z0-9])`,"i")}function de(r,e){let t=[];for(let s of e){if(s.length===0)continue;s.every(a=>j(a).test(r))&&t.push(s.join(" + "))}return t}function pe(r,e){for(let t of e)if(j(t).test(r))return!0;return!1}function Q(r,e){let t=new Map;if(r.length===0)return[];let s=new Set;document.querySelectorAll('[data-testid="list-view-item"], [data-testid="list-view-items"] > li, div[id^="issue_"], li[id^="issue_"]').forEach(o=>s.add(o)),s.size===0&&document.querySelectorAll('a[href*="/issues/"]').forEach(o=>{let a=o.closest('li, article, div[role="listitem"]');a&&s.add(a)});for(let o of s){let a=o.textContent??"";if(pe(a,e))continue;let n=de(a,r);if(n.length===0)continue;let l=o.querySelector('a[data-testid="issue-pr-title-link"], a[id^="issue_"][href*="/issues/"], a[href*="/issues/"]');if(!l)continue;let p=l.href,u=p.match(/\/issues\/(\d+)(?:[?#].*)?$/);if(!u)continue;let f=parseInt(u[1],10);if(!Number.isFinite(f)||f<=0||t.has(f))continue;let m=(l.textContent??"").trim()||`Issue #${f}`;t.set(f,{number:f,title:m,url:p,labels:n})}return Array.from(t.values())}var _=class{constructor(e,t){this.owner=e;this.repo=t}refreshTimer=null;scanTimer=null;destroyed=!1;async init(){let e=await H();if(window.location.hash==="#tasker-test-alert"&&L([{number:99999,title:"Test bounty \u2014 Bug + Daily lightning popup",url:window.location.href,labels:["Bug + Daily"]}],{sound:e.bugDailyPopupSound}),this.scanTimer=window.setTimeout(()=>{this.runScan(!0)},1500),e.autoRefreshEnabled){let t=Math.max(5,e.autoRefreshSeconds)*1e3;this.refreshTimer=window.setTimeout(()=>{this.destroyed||window.location.reload()},t)}}async runScan(e){if(!this.destroyed)try{let t=await H();await this.checkBugDailyPopup(t,e);let s=Q(t.watchedLabelGroups,t.excludedLabels),o=await O(this.owner,this.repo),a=t.notifyHelpWanted,n=s.filter(p=>!o.has(p.number));if(n.length===0)return;let l=o.size===0&&e;for(let p of n)o.add(p.number);if(await I(this.owner,this.repo,o),l||!a)return;for(let p of n.slice(0,10)){let u={type:"SEND_HELP_WANTED",owner:this.owner,repo:this.repo,number:p.number,title:p.title,url:p.url,labels:p.labels};chrome.runtime.sendMessage(u).catch(()=>{})}}catch{}}async checkBugDailyPopup(e,t){if(this.destroyed||!e.bugDailyPopupEnabled)return;let s=Q(e.watchedLabelGroups,e.excludedLabels);if(s.length===0)return;let o=await O(this.owner,this.repo,F),a=s.filter(l=>!o.has(l.number));if(a.length===0)return;let n=o.size===0&&t;for(let l of a)o.add(l.number);await I(this.owner,this.repo,o,F),!(n||this.destroyed)&&L(a,{sound:e.bugDailyPopupSound})}destroy(){this.destroyed=!0,this.refreshTimer!==null&&(clearTimeout(this.refreshTimer),this.refreshTimer=null),this.scanTimer!==null&&(clearTimeout(this.scanTimer),this.scanTimer=null)}};var x=null,R=null,B="";function ce(){return document.querySelector('[class*="sidebarContent"]')??document.querySelector(".Layout-sidebar .BorderGrid")??null}function ue(){return document.querySelector('[class*="PageHeader-Description"] .d-flex.flex-justify-between')??null}function he(){let r=document.querySelector(".js-comment-body")??document.querySelector('[data-testid="issue-body"]');if(!r)return[];let e=new Set,t=r.querySelectorAll('a[href*="/issues/"]');for(let a of t){let l=a.href.match(/\/issues\/(\d+)/);l&&e.add(parseInt(l[1],10))}let o=(r.textContent??"").matchAll(/#(\d{2,})/g);for(let a of o)e.add(parseInt(a[1],10));return Array.from(e)}function U(){let r=window.location.href;if(r===B)return;B=r,x&&(x.destroy(),x=null),R&&(R.destroy(),R=null);let e=z(r);if(e){R=new _(e.owner,e.repo),R.init();return}let t=$(r);if(!t)return;let s=(o=0)=>{if(t.type==="pr"){let a=ue();if(!a){o<20&&setTimeout(()=>s(o+1),250);return}let n=he();x=new C(t.owner,t.repo,t.number,"pr",n),a.appendChild(x.element),x.init()}else{let a=ce();if(!a){o<20&&setTimeout(()=>s(o+1),250);return}x=new C(t.owner,t.repo,t.number,"issue",[]),a.appendChild(x.element),x.init()}};s()}U();document.addEventListener("turbo:load",()=>{B="",U()});var V=window.location.href;setInterval(()=>{window.location.href!==V&&(V=window.location.href,B="",U())},1e3);chrome.runtime.onMessage.addListener(r=>{r.type==="TEST_BUG_DAILY_ALERT"&&L([{number:99999,title:"Test bounty \u2014 Bug + Daily lightning popup",url:window.location.href,labels:["Bug + Daily"]}],{sound:r.sound})});})();
