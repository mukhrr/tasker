...
Describe your fix...`,p.value=this.proposalDraftBody,p.disabled=r||this.proposalBusy,t.appendChild(p);let g=document.createElement("div");g.className="proposal-actions";let R=()=>this.proposalDraftBody!==(this.proposal?.body??"")&&this.proposalDraftBody.trim().length>0,b=document.createElement("button");b.className="proposal-btn secondary",b.textContent=this.proposalBusy?"Saving\u2026":this.proposal?"Save changes":"Save draft",b.disabled=this.proposalBusy||r||!R(),b.addEventListener("click",()=>void this.saveProposal()),g.appendChild(b);let w=null;if(r){let i=document.createElement("button");i.className="proposal-btn",i.textContent=s==="posting"?"Posting\u2026":"Disarm",i.disabled=this.proposalBusy||s==="posting",i.addEventListener("click",()=>void this.setProposalState("draft")),g.appendChild(i)}else{let i=document.createElement("button");i.className="proposal-btn primary",i.textContent=this.proposalBusy?"Arming\u2026":"Arm auto-post";let u=R();i.disabled=this.proposalBusy||!this.proposalDraftBody.trim()||u,i.title=u?"Save changes before arming":"",i.addEventListener("click",()=>void this.setProposalState("armed")),g.appendChild(i),w=i}t.appendChild(g);let P=null;if(s!=="posting"){let i=document.createElement("div");i.className="proposal-actions";let u=document.createElement("button");u.className="proposal-btn danger",u.textContent=this.proposalBusy?"Posting\u2026":"Post now";let O=this.proposalDraftBody.trim().length>0;u.disabled=this.proposalBusy||!O,u.title=O?"Post the current text as a comment immediately, without waiting for Help Wanted.":"Type your proposal first.",u.addEventListener("click",()=>void this.postProposalNow()),i.appendChild(u),t.appendChild(i),P=u}p.addEventListener("input",()=>{this.proposalDraftBody=p.value;let i=R(),u=this.proposalDraftBody.trim().length>0;b.disabled=this.proposalBusy||r||!i,w&&(w.disabled=this.proposalBusy||!u||i,w.title=i?"Save changes before arming":""),P&&(P.disabled=this.proposalBusy||!u,P.title=u?"Post the current text as a comment immediately, without waiting for Help Wanted.":"Type your proposal first.")});let y=document.createElement("div");if(y.className="proposal-status-line",r)y.textContent=s==="posting"?"Posting now\u2026":'Armed \u2014 waiting for "Help Wanted" label';else if(this.proposal){let i=this.proposal.updated_at?new Date(this.proposal.updated_at).toLocaleString():"";y.textContent=`Draft saved \xB7 ${i}`}else y.textContent='Auto-posts on "Help Wanted" via the poll worker.';if(t.appendChild(y),n&&this.proposal?.last_error){let i=document.createElement("div");i.className="proposal-error",i.textContent=`Last error: ${this.proposal.last_error}`,t.appendChild(i)}e.appendChild(t),this.root.appendChild(e)}async saveProposal(){if(this.proposalBusy)return;this.proposalBusy=!0,this.render();let e=await c({type:"SAVE_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number,body:this.proposalDraftBody});this.proposalBusy=!1,e.ok&&e.data?(this.proposal=e.data,this.proposalDraftBody=e.data.body):(this.error=e.error??"Save failed",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async setProposalState(e){if(this.proposalBusy)return;this.proposalBusy=!0,this.render();let t=await c({type:e==="armed"?"ARM_PROPOSAL":"DISARM_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number});this.proposalBusy=!1,t.ok&&t.data?(this.proposal=t.data,t.data.state==="armed"||t.data.state==="posting"?(this.startProposalPoll(),this.startFastPath()):(this.stopProposalPoll(),this.stopFastPath())):(this.error=t.error??"Update failed",setTimeout(()=>{this.error=null,this.render()},3e3)),this.render()}async postProposalNow(){if(this.proposalBusy||!this.proposalDraftBody.trim()||!confirm("Post this proposal as a comment now?"))return;this.proposalBusy=!0,this.render();let t=await c({type:"SAVE_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number,body:this.proposalDraftBody});if(!t.ok||!t.data){this.proposalBusy=!1,this.error=t.error??"Save failed",setTimeout(()=>{this.error=null,this.render()},3e3),this.render();return}this.proposal=t.data;let s=await c({type:"POST_PROPOSAL_NOW",proposalId:t.data.id,force:!0});this.proposalBusy=!1,s.ok&&s.data?(this.proposal=s.data,this.stopProposalPoll(),this.stopFastPath()):(this.error=s.error??"Post failed",setTimeout(()=>{this.error=null,this.render()},5e3),this.refreshProposal()),this.render()}startProposalPoll(){this.stopProposalPoll(),this.proposalPollHandle=setInterval(()=>{this.refreshProposal()},q)}stopProposalPoll(){this.proposalPollHandle!==null&&(clearInterval(this.proposalPollHandle),this.proposalPollHandle=null)}async refreshProposal(){if(this.destroyed)return;let e=await c({type:"QUERY_PROPOSAL",owner:this.owner,repo:this.repo,number:this.number});if(this.destroyed||!e.ok||!e.data)return;let t=e.data;(!this.proposal||t.state!==this.proposal.state||t.posted_at!==this.proposal.posted_at)&&(this.proposal=t,(t.state==="posted"||t.state==="failed"||t.state==="draft")&&(this.stopProposalPoll(),this.stopFastPath()),this.render())}startFastPath(){if(this.autoPostEnabled&&!(!this.proposal||this.proposal.state!=="armed")&&!this.fastPathTriggered){if(this.hasReadyLabel()){this.tryFastPost("initial-labels");return}this.attachFastPathObservers(),this.startFastPathPoll(),this.fastPathVisibilityListener||(this.fastPathVisibilityListener=()=>{document.visibilityState==="visible"&&this.proposal?.state==="armed"?this.startFastPathPoll():this.stopFastPathPoll()},document.addEventListener("visibilitychange",this.fastPathVisibilityListener))}}stopFastPath(){this.stopFastPathPoll();for(let e of this.fastPathObservers)e.disconnect();this.fastPathObservers=[],this.fastPathVisibilityListener&&(document.removeEventListener("visibilitychange",this.fastPathVisibilityListener),this.fastPathVisibilityListener=null),this.fastPathEtag=null}attachFastPathObservers(){let e=new Set;for(let t of Y)document.querySelectorAll(t).forEach(s=>{if(e.has(s))return;e.add(s);let r=new MutationObserver(()=>this.checkLabelsContainer(s));r.observe(s,{childList:!0,subtree:!0,characterData:!0}),this.fastPathObservers.push(r),this.checkLabelsContainer(s)})}checkLabelsContainer(e){if(this.fastPathTriggered)return;(e.innerText||e.textContent||"").toLowerCase().includes(B)&&this.tryFastPost("mutation-observer")}startFastPathPoll(){this.fastPathHandle===null&&document.visibilityState==="visible"&&(this.fastPathHandle=setInterval(()=>{this.fastPollTick()},V),this.fastPollTick())}stopFastPathPoll(){this.fastPathHandle!==null&&(clearInterval(this.fastPathHandle),this.fastPathHandle=null)}async fastPollTick(){if(this.destroyed||this.fastPathTriggered)return;if(!this.proposal||this.proposal.state!=="armed"){this.stopFastPath();return}let e=await c({type:"QUERY_ISSUE_LABELS_ETAG",owner:this.owner,repo:this.repo,number:this.number,etag:this.fastPathEtag});!e.ok||!e.data||(e.data.etag&&(this.fastPathEtag=e.data.etag),!(e.data.notModified||!e.data.labels)&&(this.labels=e.data.labels,this.hasReadyLabel()&&this.tryFastPost("etag-poll")))}async tryFastPost(e){if(console.debug("[tasker] fast-post triggered via",e),this.fastPathTriggered||!this.autoPostEnabled||!this.proposal||this.proposal.state!=="armed")return;this.fastPathTriggered=!0,this.stopFastPath();let t=this.proposal.id;this.proposal={...this.proposal,state:"posting"},this.render();let s=await c({type:"POST_PROPOSAL_NOW",proposalId:t});this.destroyed||(s.ok&&s.data?(this.proposal=s.data,this.stopProposalPoll()):(this.fastPathTriggered=!1,this.refreshProposal()),this.render())}groupStatuses(){let e={todo:[],in_progress:[],complete:[]};for(let t of this.statuses)e[t.group_name]?.push(t);for(let t of E)e[t].sort((s,r)=>s.position-r.position);return e}escapeHtml(e){let t=document.createElement("span");return t.textContent=e,t.innerHTML}destroy(){this.destroyed=!0,this.stopProposalPoll(),this.stopFastPath(),this.container.remove()}getHeaderStyles(){return`
    ...
    `}};var D="extensionSettings";var f={autoRefreshEnabled:!1,autoRefreshSeconds:20,notifyHelpWanted:!1,notifyChannels:["browser"],telegramChatId:"",pollSeconds:45,watchedLabelGroups:[["Help Wanted"],["Daily"],["Bug"]],excludedLabels:["DeployBlocker","DeployBlockerCash"]},j=["browser","telegram"];function Q(a){if(!Array.isArray(a))return[...f.notifyChannels];let e=a.filter(t=>typeof t=="string"&&j.includes(t));return e.length>0?Array.from(new Set(e)):[...f.notifyChannels]}function A(a,e){if(!Array.isArray(a))return[...e];let t=a.filter(s=>typeof s=="string").map(s=>s.trim()).filter(s=>s.length>0&&s.length<=64);return Array.from(new Map(t.map(s=>[s.toLowerCase(),s])).values())}function K(a){if(!Array.isArray(a))return null;let e=[];for(let t of a){if(!Array.isArray(t))continue;let s=A(t,[]);s.length>0&&e.push(s)}return e}async function _(){let e=(await chrome.storage.local.get(D))[D],t,s=K(e?.watchedLabelGroups);return s!==null?t=s:e?.watchedLabels!==void 0?t=A(e.watchedLabels,[]).map(r=>[r]):t=f.watchedLabelGroups.map(r=>[...r]),{autoRefreshEnabled:e?.autoRefreshEnabled??f.autoRefreshEnabled,autoRefreshSeconds:Math.max(5,e?.autoRefreshSeconds??f.autoRefreshSeconds),notifyHelpWanted:e?.notifyHelpWanted??f.notifyHelpWanted,notifyChannels:Q(e?.notifyChannels),telegramChatId:e?.telegramChatId??f.telegramChatId,pollSeconds:Math.max(30,e?.pollSeconds??f.pollSeconds),watchedLabelGroups:t,excludedLabels:e?.excludedLabels===void 0?[...f.excludedLabels]:A(e.excludedLabels,[])}}function U(a,e){return`seenHelpWanted:${a.toLowerCase()}/${e.toLowerCase()}`}async function $(a,e){let t=U(a,e),r=(await chrome.storage.local.get(t))[t]??[];return new Set(r)}async function z(a,e,t){let s=Array.from(t),r=s.length>500?s.slice(s.length-500):s;await chrome.storage.local.set({[U(a,e)]:r})}function X(a){return a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function F(a){let e=X(a).replace(/\s+/g,"[\\s-]?");return new RegExp(`(?:^|[^a-z0-9])${e}(?:$|[^a-z0-9])`,"i")}function J(a,e){let t=[];for(let s of e){if(s.length===0)continue;s.every(o=>F(o).test(a))&&t.push(s.join(" + "))}return t}function Z(a,e){for(let t of e)if(F(t).test(a))return!0;return!1}function ee(a,e){let t=new Map;if(a.length===0)return[];let s=new Set;document.querySelectorAll('[data-testid="list-view-item"], [data-testid="list-view-items"] > li, div[id^="issue_"], li[id^="issue_"]').forEach(r=>s.add(r)),s.size===0&&document.querySelectorAll('a[href*="/issues/"]').forEach(r=>{let o=r.closest('li, article, div[role="listitem"]');o&&s.add(o)});for(let r of s){let o=r.textContent??"";if(Z(o,e))continue;let n=J(o,a);if(n.length===0)continue;let l=r.querySelector('a[data-testid="issue-pr-title-link"], a[id^="issue_"][href*="/issues/"], a[href*="/issues/"]');if(!l)continue;let d=l.href,h=d.match(/\/issues\/(\d+)(?:[?#].*)?$/);if(!h)continue;let p=parseInt(h[1],10);if(!Number.isFinite(p)||p<=0||t.has(p))continue;let g=(l.textContent??"").trim()||`Issue #${p}`;t.set(p,{number:p,title:g,url:d,labels:n})}return Array.from(t.values())}var T=class{constructor(e,t){this.owner=e;this.repo=t}refreshTimer=null;scanTimer=null;destroyed=!1;async init(){this.scanTimer=window.setTimeout(()=>{this.runScan(!0)},1500);let e=await _();if(e.autoRefreshEnabled){let t=Math.max(5,e.autoRefreshSeconds)*1e3;this.refreshTimer=window.setTimeout(()=>{this.destroyed||window.location.reload()},t)}}async runScan(e){if(!this.destroyed)try{let t=await _(),s=ee(t.watchedLabelGroups,t.excludedLabels),r=await $(this.owner,this.repo),o=t.notifyHelpWanted,n=s.filter(d=>!r.has(d.number));if(n.length===0)return;let l=r.size===0&&e;for(let d of n)r.add(d.number);if(await z(this.owner,this.repo,r),l||!o)return;for(let d of n.slice(0,10)){let h={type:"SEND_HELP_WANTED",owner:this.owner,repo:this.repo,number:d.number,title:d.title,url:d.url,labels:d.labels};chrome.runtime.sendMessage(h).catch(()=>{})}}catch{}}destroy(){this.destroyed=!0,this.refreshTimer!==null&&(clearTimeout(this.refreshTimer),this.refreshTimer=null),this.scanTimer!==null&&(clearTimeout(this.scanTimer),this.scanTimer=null)}};var m=null,v=null,L="";function te(){return document.querySelector('[class*="sidebarContent"]')??document.querySelector(".Layout-sidebar .BorderGrid")??null}function se(){return document.querySelector('[class*="PageHeader-Description"] .d-flex.flex-justify-between')??null}function re(){let a=document.querySelector(".js-comment-body")??document.querySelector('[data-testid="issue-body"]');if(!a)return[];let e=new Set,t=a.querySelectorAll('a[href*="/issues/"]');for(let o of t){let l=o.href.match(/\/issues\/(\d+)/);l&&e.add(parseInt(l[1],10))}let r=(a.textContent??"").matchAll(/#(\d{2,})/g);for(let o of r)e.add(parseInt(o[1],10));return Array.from(e)}function H(){let a=window.location.href;if(a===L)return;L=a,m&&(m.destroy(),m=null),v&&(v.destroy(),v=null);let e=M(a);if(e){v=new T(e.owner,e.repo),v.init();return}let t=N(a);if(!t)return;let s=(r=0)=>{if(t.type==="pr"){let o=se();if(!o){r<20&&setTimeout(()=>s(r+1),250);return}let n=re();m=new x(t.owner,t.repo,t.number,"pr",n),o.appendChild(m.element),m.init()}else{let o=te();if(!o){r<20&&setTimeout(()=>s(r+1),250);return}m=new x(t.owner,t.repo,t.number,"issue",[]),o.appendChild(m.element),m.init()}};s()}H();document.addEventListener("turbo:load",()=>{L="",H()});var W=window.location.href;setInterval(()=>{window.location.href!==W&&(W=window.location.href,L="",H())},1e3);})();
